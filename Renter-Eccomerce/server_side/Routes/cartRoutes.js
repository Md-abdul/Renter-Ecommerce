...
CartRoutes.post("/add", verifyToken, async (req, res) => {
  try {
    ...
    const existingItemEntry = [...user.cart.entries()].find(
      ([, item]) =>
    ...
    let itemId =
      existingItemEntry?.[0] || new mongoose.Types.ObjectId().toString();
    if (existingItemEntry) {
      totalQuantity += existingItemEntry[1].quantity;
      if (totalQuantity > sizeObj.quantity) {
        return res
          .status(400)
          .json({ message: "Exceeds stock", available: sizeObj.quantity });
      }
    }
